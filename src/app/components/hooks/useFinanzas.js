...
export const useFinanzas = () => {
  ...
  const crearNuevoAnio = async (anio) => {
    if (!datosAnios[anio]) {
      let nuevaEstructura = crearEstructuraAnio(anio);
      const anioAnterior = anio - 1;
      const mesesAnioAnterior = datosAnios[anioAnterior]?.meses;

      if (mesesAnioAnterior?.length) {
        const diciembreAnterior = mesesAnioAnterior[11];
        nuevaEstructura = {
          ...nuevaEstructura,
          meses: propagarRecurrentesDesdeMes(diciembreAnterior, nuevaEstructura.meses),
        };
      }
  ...
  const actualizarMontoQuincenal = (quincena, categoria, index, nuevoMonto) => {
    setDatosAnios((prev) => {
      ...
      const itemActual = mes.datosQuincenales[quincena][categoria][index];
      ...
      if (itemActual.recurrente) {
        anio.meses = actualizarMontoRecurrenteEnMesesSiguientes(
          anio.meses,
          mesSeleccionado,
          quincena,
          categoria,
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          const dataSincronizada = Object.fromEntries(
            Object.entries(data).map(([anio, anioData]) => [
              anio,
              {
                ...anioData,
          ...
          setDatosAnios(dataSincronizada);
        } catch (error) {
